...
async function migrate() {
  try {
    ...
    const existingCount = await db.prepare('SELECT COUNT(*) as count FROM categories').get();
    if (existingCount.count > 0) {
      console.log(`分类表已有 ${existingCount.count} 条记录，跳过插入`);
      return;
    }
    ...
    const categories = [
      {
        ...
        value: 'portrait',
        description: '人像摄影',
        analysis_prompt: `你是一位专业人像摄影指导专家。请分析这张人像照片的构图质量，并给出具体、可操作的调整建议。
...
注意：只返回JSON，不要包含其他文字。`,
        comparison_prompt: `你是一位专业人像摄影指导专家。用户希望拍摄出与参考图类似效果的人像照片。
...
注意：只返回JSON，不要包含其他文字。`
      },
      {
        ...
        value: 'landscape',
        description: '风景摄影',
        analysis_prompt: `你是一位专业风景摄影指导专家。请分析这张风景照片的构图质量，并给出具体、可操作的调整建议。
...
注意：只返回JSON，不要包含其他文字。`,
        comparison_prompt: `你是一位专业风景摄影指导专家。用户希望拍摄出与参考图类似效果的风景照片。
...
注意：只返回JSON，不要包含其他文字。`
      },
      {
        ...
        value: 'food',
        description: '美食摄影',
        analysis_prompt: `你是一位专业美食摄影指导专家。请分析这张美食照片的构图质量，并给出具体、可操作的调整建议。
...
注意：只返回JSON，不要包含其他文字。`,
        comparison_prompt: `你是一位专业美食摄影指导专家。用户希望拍摄出与参考图类似效果的美食照片。
...
注意：只返回JSON，不要包含其他文字。`
      },
      {
        ...
        value: 'pet',
        description: '宠物摄影',
        analysis_prompt: `你是一位专业宠物摄影指导专家。请分析这张宠物照片的构图质量，并给出具体、可操作的调整建议。
...
注意：只返回JSON，不要包含其他文字。`,
        comparison_prompt: `你是一位专业宠物摄影指导专家。用户希望拍摄出与参考图类似效果的宠物照片。
...
注意：只返回JSON，不要包含其他文字。`
      },
      {
        ...
        value: 'architecture',
        description: '建筑摄影',
        analysis_prompt: `你是一位专业建筑摄影指导专家。请分析这张建筑照片的构图质量，并给出具体、可操作的调整建议。
...
注意：只返回JSON，不要包含其他文字。`,
        comparison_prompt: `你是一位专业建筑摄影指导专家。用户希望拍摄出与参考图类似效果的建筑照片。
...
注意：只返回JSON，不要包含其他文字。`
      },
      {
        ...
        value: 'street',
        description: '街头摄影',
        analysis_prompt: `你是一位专业街头摄影指导专家。请分析这张街拍照片的构图质量，并给出具体、可操作的调整建议。
...
注意：只返回JSON，不要包含其他文字。`,
        comparison_prompt: `你是一位专业街头摄影指导专家。用户希望拍摄出与参考图类似效果的街拍照片。
...
注意：只返回JSON，不要包含其他文字。`
      },
      {
        ...
        value: 'still_life',
        description: '静物摄影',
        analysis_prompt: `你是一位专业静物摄影指导专家。请分析这张静物照片的构图质量，并给出具体、可操作的调整建议。
...
注意：只返回JSON，不要包含其他文字。`,
        comparison_prompt: `你是一位专业静物摄影指导专家。用户希望拍摄出与参考图类似效果的静物照片。
...
注意：只返回JSON，不要包含其他文字。`
      },
      {
        ...
        value: 'custom',
        description: '自定义参考图',
        analysis_prompt: `你是一位专业摄影指导专家。请分析这张照片的构图质量，并给出具体、可操作的调整建议。
...
注意：只返回JSON，不要包含其他文字。`,
        comparison_prompt: `你是一位专业摄影指导专家。用户希望拍摄出与参考图类似效果的照片。
...
注意：只返回JSON，不要包含其他文字。`
    ...
    for (const cat of categories) {
      await db.prepare(`
        INSERT INTO categories (id, name, value, description, analysis_prompt, comparison_prompt)
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(cat.id, cat.name, cat.value, cat.description, cat.analysis_prompt, cat.comparison_prompt);
      console.log(`✅ 分类已插入: ${cat.name} (${cat.value})`);
    }

    console.log(`\n✅ 迁移完成！共插入 ${categories.length} 个分类`);
  } catch (error) {
